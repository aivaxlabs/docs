...
- Autenticação rejeita chaves de API ausentes, expiradas ou desconhecidas.
- Chaves de API públicas são restritas a rotas públicas e têm limites de requisição e token por chave e por IP.
- Middleware de saldo rejeita requisições faturáveis quando o saldo da conta está abaixo do mínimo necessário.
- Middleware de armazenamento rejeita requisições quando o armazenamento da conta excede a cota do plano.
- Inferência verifica o acesso ao modelo, taxa de requisição, taxa de tokens de entrada, taxa BYOK e tamanho de contexto do plano Free.
...
| Reserva de modelo de assinatura | Não incluído | 250 unidades/6h e 3.000 unidades/semana | 1.000 unidades/6h e 15.000 unidades/semana |
| Requisições de texto-para-fala e transcrição de áudio | 3/min e 15/hora | 30/min | 300/min |
| **RAG e coleções** |  |  |  |
| Coleções | 5 | — | — |
...
| Processamento de arquivo composto | Não disponível | 3 arquivos/dia | 10 arquivos/dia |
...
Requisições de modelo integrado são limitadas tanto por contagem de requisições quanto por tokens de entrada. Grupos de limite de taxa de modelo ajustam os limiares de contagem de requisições:
...
Texto-para-fala e transcrição de áudio autônoma compartilham a mesma cota de requisições do plano. Sessões de voz consomem essa cota quando iniciam a síntese de fala; sua transcrição e inferência conversacional são contabilizadas como parte da sessão e não têm uma cota de plano separada.

O endpoint de importação JSONL rejeita uma requisição quando atinge o limite de documentos por requisição do plano. O limite de reclassificação se aplica ao endpoint de reclassificação autônoma e às buscas RAG que usam um reclassificador, incluindo buscas realizadas através de AI Gateways e ferramentas MCP.

O limite Reflex conta o tempo gasto processando requisições Reflex. Ele se aplica ao endpoint de reclassificação autônoma e às buscas RAG que usam Reflex; entrada em cache não consome a cota separadamente. Requisições que excedem o limite do plano retornam `429 Too Many Requests`. Veja [Reflex](rag/reflex.md) para limites de requisição, comportamento de cache e preços.
