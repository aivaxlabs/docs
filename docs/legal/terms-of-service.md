...
- **Semantic Training Data:** Anonymized eligible RAG and reranking content collected after the AIVAX Account Manager enables semantic data collection, as described in [Data Collecting](/docs/data-collecting). Document indexing and storage are excluded.
...
- authorizes AIVAX to collect eligible anonymized RAG and reranking records generated while the setting is enabled;
...
- acknowledges that eligible RAG and reranking operations receive the documented 10% discount, while document indexing, storage, and unrelated services remain at their regular prices.

AIVAX anonymizes these records before storage by removing the account relationship and excluding operational identifiers, document names, billing data, and collection timestamps.
