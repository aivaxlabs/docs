...
- **Semantic Training Data:** Anonymized eligible RAG and reranking content collected after the Account Manager enables semantic data collection, as described in [Data Collecting](/docs/data-collecting). Document indexing and storage are excluded.
...
4. **Semantic Training Data:** Anonymized RAG query terms, contents and relevance scores of documents returned by a search, selected reranker, reranking queries, submitted documents, and ranking results collected while semantic data collection is enabled. Document indexing and storage are not collected. These anonymized records exclude account, API key, request, collection, and document identifiers, document names, billing data, and collection timestamps.
...
| Semantic Training Data | Develop, train, fine-tune, evaluate, test, and improve models and semantic retrieval or ranking systems | Consent | Retained in anonymized form while reasonably necessary for these purposes, legal obligations, security, and audits; then deleted |
