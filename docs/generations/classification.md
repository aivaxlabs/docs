...
Embedding usage is billed for text that requires inference and is associated with the authenticated API key. Repeated text may be served from the account-scoped embedding cache. Because the endpoint returns every document-label pair, response size and comparison work grow with `documents × labels`.
