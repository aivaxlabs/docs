...
This MCP is read-oriented. It does not expose a generic account API invocation tool. Use it when an agent needs to understand AIVAX features, find the right API route, compare model capabilities, or ground its answer in the product manual. Use the [account management MCP](/docs/mcp-utilities/account-management-mcp) only when the client also needs to inspect or change authenticated account resources through API calls.

> [!NOTE]
> Do not configure the documentation MCP together with the [account management MCP](/docs/mcp-utilities/account-management-mcp) in the same client unless you have a specific reason to duplicate tools. The account management MCP already includes documentation search functions, so adding both servers usually creates redundant documentation tools and can make tool selection less predictable.
